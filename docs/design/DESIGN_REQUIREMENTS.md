...
| FR-9.1.3 | History retention _(partial)_ | Manual clear option; configurable history size limit, default 500 entries, adjustable to 10,000 from the history panel. History lives in `localStorage`, so the real ceiling is that origin's few megabytes rather than a count. Age-based retention is not implemented.                   |
