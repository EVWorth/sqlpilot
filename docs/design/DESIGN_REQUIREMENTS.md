...
| FR-7.1.6 | Role management         | Create and drop roles, grant and revoke them, and set or clear a user's default role, from the Roles tab. The catalog queries and the `SET DEFAULT ROLE` spelling both fork between MySQL and MariaDB; see `lib/admin/roles.ts`.                                                                                               |
...
| FR-7.2.2 | Server variables       | Browse, search and group; edit a settable variable in place with a choice of session, global, or persisted scope. A global change warns that it reverts on restart. Descriptions come from `information_schema.SYSTEM_VARIABLES` on MariaDB; MySQL exposes none over SQL, so none are shown rather than invented. `SET PERSIST` is offered on MySQL only — MariaDB has no equivalent. |
| FR-7.2.3 | Server status          | Display key server metrics: uptime, threads, queries/second, connections, buffer pool usage                                                                                                                                                                                                                                                                                           |
| FR-7.2.4 | Replication monitoring | Shows whichever side the server is on — replica status with thread state, lag and last errors; binary log position; connected replicas. Statement names and every status column were renamed in MySQL 8.0.22 and kept as-is by MariaDB, so both vocabularies are read and both statement spellings are tried.                                                                         |
...
| FR-9.1.2 | Search history              | Substring search across all recorded queries; filter by connection, database, date range, success/failure and minimum duration; sort by recency, duration or row count; export the filtered view as CSV or SQL. Filtering is a `WHERE` clause against `history.db`, not an in-memory scan.                                                                                                                                                                                                                                                          |
| FR-9.1.3 | History retention           | Manual clear; a configurable size limit (default 500, up to 10,000) and a configurable retention period (default: keep forever, up to 365 days), both set from the history panel. Both apply and whichever bites first wins. Age retention is applied at startup, not on a timer, so a long-running session does not delete rows under the user. An optional "hide values in new entries" setting, off by default, blanks every literal before storing for shared or regulated machines; credential redaction is always on and is not a preference. |
| FR-9.1.4 | Re-execute from history     | Click an entry to load it into the editor; right-click for "Run now", which loads it and runs it against the connection the entry came from rather than whichever is selected. Disabled when that connection is closed, or when a credential was redacted out of the statement.                                                                                                                                                                                                                                                                     |
...
_This document is a living specification. It will be updated as the project evolves and new requirements are identified._
