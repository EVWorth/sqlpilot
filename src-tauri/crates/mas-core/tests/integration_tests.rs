...
async fn test_test_connection_failure() {
    ...
    assert!(
        msg_lower.contains("fail") || msg_lower.contains("denied") || msg_lower.contains("access") || msg_lower.contains("error"),
...
async fn test_execute_select() {
    ...
    let results = executor.execute(&info.id, "SELECT 1 AS num, 'hello' AS greeting").await.unwrap();
...
async fn test_execute_select_from_table() {
    ...
    let results = executor.execute(&info.id, "SELECT id, username, email FROM users ORDER BY id").await.unwrap();
    assert_eq!(results.len(), 1);
    assert!(results[0].rows.len() >= 5, "Expected at least 5 users, got {}", results[0].rows.len());
...
async fn test_execute_insert_update_delete() {
    ...
    // Insert
    let results = executor.execute(&info.id,
...
async fn test_execute_multi_statement() {
    ...
    let results = executor.execute(&info.id, "SELECT 1; SELECT 2; SELECT 3").await.unwrap();
    assert_eq!(results.len(), 3, "Expected 3 result sets for 3 statements, got {}", results.len());
...
async fn test_execute_show_commands() {
    ...
    let results = executor.execute(&info.id, "SHOW DATABASES").await.unwrap();
    assert_eq!(results.len(), 1);
    assert!(!results[0].rows.is_empty());

    let results = executor.execute(&info.id, "SHOW TABLES").await.unwrap();
    assert_eq!(results.len(), 1);
    assert!(!results[0].rows.is_empty());
...
async fn test_execute_with_error() {
    ...
    let result = executor.execute(&info.id, "SELECT * FROM nonexistent_table").await;
    assert!(result.is_err());
...
async fn test_execute_ddl() {
    ...
    // Create table
    let _ = executor.execute(&info.id, "DROP TABLE IF EXISTS test_temp").await;
    let results = executor.execute(&info.id,
    ...
    // Insert and select
    executor.execute(&info.id, "INSERT INTO test_temp VALUES (1, 'test')").await.unwrap();
    let results = executor.execute(&info.id, "SELECT * FROM test_temp").await.unwrap();
    assert_eq!(results[0].rows.len(), 1);

    // Clean up
    executor.execute(&info.id, "DROP TABLE test_temp").await.unwrap();
...
async fn test_null_handling() {
    ...
    let results = executor.execute(&info.id, "SELECT NULL AS null_val, 'not null' AS str_val").await.unwrap();
    assert_eq!(results[0].rows.len(), 1);
...
async fn test_data_types() {
    ...
    assert_eq!(results[0].rows.len(), 1);
    assert!(results[0].columns.len() > 20, "Expected many columns, got {}", results[0].columns.len());
...
async fn test_unicode_data() {
    ...
    let results = executor.execute(&info.id, "SELECT id, content_utf8, content_emoji FROM unicode_test").await.unwrap();
    assert!(results[0].rows.len() >= 6, "Expected at least 6 unicode rows, got {}", results[0].rows.len());
    ...
    let has_japanese = results[0].rows.iter().any(|row| {
        row.iter().any(|v| match v {
            ...
            mas_core::models::SqlValue::Bytes(b) => {
                String::from_utf8_lossy(b).contains("日本語")
            },
            _ => false,
        })
    });
...
async fn test_json_data() {
    ...
    match val {
        mas_core::models::SqlValue::String(s) => {
            assert!(s.contains("{"), "JSON data should be returned as string containing '{{', got: {}", s);
        },
...
async fn test_list_databases() {
    ...
    assert!(db_names.contains(&"test_db"), "Should contain test_db, got: {:?}", db_names);
    assert!(db_names.contains(&"test_db_empty"), "Should contain test_db_empty, got: {:?}", db_names);
...
async fn test_list_tables() {
    ...
    assert!(table_names.contains(&"users"), "Should contain users table, got: {:?}", table_names);
    assert!(table_names.contains(&"orders"), "Should contain orders table, got: {:?}", table_names);
    assert!(table_names.contains(&"products"), "Should contain products table, got: {:?}", table_names);
    assert!(table_names.contains(&"categories"), "Should contain categories table, got: {:?}", table_names);
...
async fn test_list_views() {
    ...
    let tables = inspector.get_tables(&info.id, "test_db").await.unwrap();
    let views: Vec<&str> = tables.iter()
        .filter(|t| t.table_type == "VIEW")
        .map(|t| t.name.as_str())
        .collect();
    assert!(views.contains(&"active_users"), "Should contain active_users view, got: {:?}", views);
    assert!(views.contains(&"order_summary"), "Should contain order_summary view, got: {:?}", views);
...
async fn test_list_columns() {
    ...
    let columns = inspector.get_columns(&info.id, "test_db", "users").await.unwrap();
    ...
    let email_col = columns.iter().find(|c| c.name == "email").expect("Should have email column");
...
async fn test_list_indexes() {
    ...
    let indexes = inspector.get_indexes(&info.id, "test_db", "users").await.unwrap();
    assert!(!indexes.is_empty(), "Should have at least one index");

    let pk = indexes.iter().find(|i| i.name == "PRIMARY").expect("Should have PRIMARY index");
    assert!(pk.is_unique);
...
async fn test_get_table_ddl() {
    ...
    let ddl = inspector.get_table_ddl(&info.id, "test_db", "users").await.unwrap();
    assert!(ddl.contains("CREATE TABLE"), "DDL should contain CREATE TABLE, got: {}", &ddl[..100.min(ddl.len())]);
...
async fn test_schema_empty_database() {
    ...
    let tables = inspector.get_tables(&info.id, "test_db_empty").await.unwrap();
...
async fn test_export_csv() {
    ...
    let results = executor.execute(&info.id, "SELECT id, username, email FROM users LIMIT 3").await.unwrap();
    let csv = mas_export::export_csv(&results[0]);

    assert!(csv.starts_with("id,username,email\n"), "CSV should start with header, got: {}", &csv[..50.min(csv.len())]);
    let lines: Vec<&str> = csv.trim().lines().collect();
    assert_eq!(lines.len(), 4, "Expected header + 3 data rows, got {} lines", lines.len());
...
async fn test_export_json() {
    ...
    let results = executor.execute(&info.id, "SELECT id, username FROM users LIMIT 2").await.unwrap();
...
async fn test_export_sql_insert() {
    ...
    let results = executor.execute(&info.id, "SELECT id, username FROM users LIMIT 1").await.unwrap();
    let sql = mas_export::export_sql_insert(&results[0], "users");

    assert!(sql.contains("INSERT INTO `users`"), "SQL should contain INSERT INTO `users`, got: {}", sql);
    assert!(sql.contains("`id`"), "SQL should contain `id` column");
    assert!(sql.contains("`username`"), "SQL should contain `username` column");
...
async fn test_export_markdown() {
    ...
    assert!(md.contains("| id | username |"), "Markdown should contain header, got: {}", md);
...
async fn test_get_process_list() {
    ...
    assert!(processes.iter().any(|p| p.user.contains("root")), "Should see root user process");
...
async fn test_get_server_variables() {
    ...
    assert!(vars.iter().any(|v| v.name == "version"), "Should have version variable");
